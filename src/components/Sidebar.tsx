...
class Sidebar extends Component<SidebarProps, SidebarState> {
  ...
  render() {
    const { title, showSearch = false, menuItems = [], bgColor = 'bg-stone-500', color = 'text-white', isCollapsed, activePath, onMenuItemClick } = this.props;
        ...
        {({ searchTerm, setSearchTerm }) => {
          ...
          return (
            <aside
              ...
              className={`${bgColor} ${color} ${isCollapsed && !isHovered ? 'w-20' : 'w-64'} h-full p-4 transition-all duration-300 ease-in-out border border-stone-200 rounded-lg shadow-lg shadow-stone-300/40`}>
              {/* Judul Sidebar */}
              ...
              {/* Input Pencarian */}
              {showSearch && (
                <div className={`mb-4 delay-100 ${isCollapsed && !isHovered ? 'hidden' : ''}`}>
                  <div className="flex items-center border border-blue-400 rounded-lg bg-blue-100">
                    <svg style={{ marginLeft: 10 }} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
